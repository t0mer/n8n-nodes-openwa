...
It sends text, media, locations, polls, contact cards, products and templates to **contacts** and **groups**, one by one or as gateway-side bulk batches; replies to, reacts to, forwards, edits, deletes, pins and stars messages; reads message history and downloads media. It also covers the rest of the WhatsApp account and the gateway:

- **WhatsApp**: calls (reject, call links), the business catalog, channels (newsletters), chats (read state, archive, pin, mute, presence), contacts, groups (participants, admins, join requests, invite links, settings, picture), labels, media conversion (voice notes, video), the profile, status updates (stories) and text templates.
- **Gateway administration**: sessions (create, start, stop, QR and pairing codes, config, proxy), webhooks, server-side automation rules (auto-replies), API keys, and system statistics, settings, audit log, message search and health checks.
- **Triggers** for message, session, group, call, status and presence events.

It can also be used as a tool by n8n AI Agents.
...
- OpenWA 0.23.5 is the tested server version; see [Server compatibility](#server-compatibility).
...
The key's role and restrictions decide which operations work; see [API key roles](#api-key-roles).

## Operations

The OpenWA node has 17 resources, listed here in two groups and alphabetically within each group, like the node's Resource list:

- **WhatsApp**: [Call](#call), [Catalog](#catalog), [Channel](#channel), [Chat](#chat), [Contact](#contact), [Group](#group), [Label](#label), [Media](#media), [Message](#message), [Profile](#profile), [Status](#status), [Template](#template).
- **Gateway administration**: [API Key](#api-key), [Automation Rule](#automation-rule), [Session](#session), [System](#system), [Webhook](#webhook).

Fields shared by many operations (Session, Recipient Type, Media Source, Message ID) are described under [Message](#message).
Operations limited to one WhatsApp engine are collected in [Engine support](#engine-support).
...
Every OpenWA API key has one of three roles. The node sends the key from the credentials with every request, so the role decides what the node can do.

| Role | Can do |
|---|---|
| **Viewer** | Read-only operations: Get, Get Many, Get Stats, Download Media, Check Number, and so on. |
| **Operator** | Everything a viewer can, plus sending messages and changing things: chats, contacts, groups, labels, channels, status, profile, sessions, templates, webhooks and automation rules. |
| **Admin** | Everything an operator can, plus managing API keys (every **API Key** operation except Validate). |

The OpenWA API reference explicitly marks only the API key endpoints as admin-only, and names the operator role for a few write routes (e.g. deleting a status). The viewer/operator split above follows OpenWA's role model; if an operation is refused for a role, the gateway's 403 message says so. Use **API Key → Validate** to see the role of the key in the credentials.

A key can also be restricted, whatever its role (set with **API Key → Create** or **Update**):

- **Allowed IPs**: addresses or CIDR ranges the key may be used from. n8n's outgoing address must be in the list.
- **Allowed Sessions**: session IDs (the UUIDs, not session names) the key may act on. A name never matches, so a key restricted by name reaches no session.
- **Allowed Chats**: chats the key may read and send to: groups (`@g.us`), contacts (`@c.us` or `@lid`) or bare phone numbers. A key restricted this way can't use Get Batch Status or Cancel Batch.

An empty list means no restriction.

**HTTP 403** means the request was understood but not allowed: the key's role is too low, the request falls outside the key's allowed IPs, sessions or chats, or WhatsApp itself refused the change (most often because the session account isn't a group or channel admin). The node shows the gateway's message, which tells these apart, together with a short roles hint. A wrong or revoked key is a 401 instead.

## Engine support

OpenWA runs each session on one of two WhatsApp engines, **Baileys** or **whatsapp-web.js**. Most operations work on both; the ones below work on only one. On the other engine the gateway answers **HTTP 501** ("not supported by the active engine"), and the node shows the gateway's message.

| Resource | Operation or option | Engine |
|---|---|---|
| Call | Reject | Baileys only |
| Catalog | Get, Get Products, Get Product | Baileys only |
| Channel | Get Many, Get Messages | whatsapp-web.js only |
| Channel | Subscribe, Demote Admin, Transfer Ownership | Baileys only |
| Chat | Subscribe to Presence | Baileys only |
| Group | Create | Baileys only |
| Label | Get Many, Get (and the label list in the Label field) | whatsapp-web.js only |
| Label | Create or Update, Delete | Baileys only |
| Label | Get Chats | whatsapp-web.js only |
| Message | Send Product | Baileys only |
| Message | Vote Poll | whatsapp-web.js only |
| Message | Send Image, Video, Audio, Document or Sticker to a channel (`@newsletter`) | Baileys only |
| Message | Get Chat History → Deep | whatsapp-web.js only |
| Session | Create → Auto Reject Calls, Update Config → Auto Reject Calls | Baileys only |
| OpenWA Call Trigger | `call.accepted`, `call.rejected`, `call.missed` | Baileys only |

Some operations work on both engines but behave differently:

- **Status → Post** (Recipients): Baileys posts only to the Recipients list; whatsapp-web.js ignores it and uses the account's status privacy settings.
- **Message → Star / Unstar**: best-effort on whatsapp-web.js, which may ignore it silently.
- **Send Text → Link Preview**: whatsapp-web.js shows previews by default; Baileys only when the option is on.
- **Chat → Mark as Read**: on Baileys, list the Message IDs to acknowledge each message.
- Renames, picture changes and pins: whatsapp-web.js reports WhatsApp's refusal as a 403, while Baileys has no acceptance signal and answers success.

The engine list reflects OpenWA 0.23.5. A newer server may support more.

## Server compatibility

The node is tested against **OpenWA server 0.23.5**. Most operations also work on older 0.23.x servers.

- **Session → Get Proxy / Update Proxy** need OpenWA **0.23.4** or newer.
- An operation the server doesn't have yet answers **HTTP 404** with the gateway's routing message, e.g. `Cannot GET /api/sessions/…/proxy`. The node shows that message with a hint to check that the Base URL points at the OpenWA gateway (without `/api`). If the Base URL is right and other operations work, the server is older than the operation needs: upgrade OpenWA.
- Use **System → Get Health** to see the server's `version`.
